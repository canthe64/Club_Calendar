...
/// </summary>
public partial class SheetBookingService(IGraphEventGateway graph, IMemoryCache cache, FacilityConfiguration facility, AppLogService log, ViewCacheRegistry viewCache, SchedulingWindowService window)
{
    ...
    // Settings.razor's own dropdown only ever offers up to 120 (D55) - this is a generous service-
    // layer backstop, not a value staff can actually reach through the UI, catching a nonsensical
    // value (e.g. days' worth of minutes) from any other caller the same way the lower bound already
    // does (code review C8).
    private const int MaxMinimumGroupEventBookingIntervalMinutes = 1440;
    ...
    public async Task SetMinimumGroupEventBookingIntervalAsync(int minutes, string actor, CancellationToken ct = default)
    {
        ...
        if (minutes > MaxMinimumGroupEventBookingIntervalMinutes)
        {
            throw new ArgumentOutOfRangeException(nameof(minutes), $"Minimum interval cannot exceed {MaxMinimumGroupEventBookingIntervalMinutes} minutes.");
        }
    ...
    // External-booking-source support (FindByExternalIdAsync/ClaimHoldAsync/TrimHoldAsync/
    // ForceCreateConfirmedAsync) now lives in SheetBookingService.ExternalSource.cs, a partial
    // class split along this file's own pre-existing comment-banner boundary (code review O8).
    ...
    // IReadOnlyList<SheetBooking>, not List<SheetBooking> (code review C5) - the returned instance is
    // the cached one, shared by every concurrent caller for the rest of this cache entry's TTL; a
    // future caller reaching for .Sort()/.Add()/.RemoveAll() would silently corrupt what every other
    // view and the public endpoints read next. No caller here mutates it today, but the type change
    // makes that structurally impossible rather than relying on nobody ever doing it.
    public async Task<IReadOnlyList<SheetBooking>> GetBookingsForAllSheetsAsync(DateTime start, DateTime end, CancellationToken ct = default)
    {
